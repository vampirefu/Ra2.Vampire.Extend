#!/usr/bin/env python3
"""Disassemble specific functions from gamemd.exe"""
import struct
import capstone
...
def disasm_function(data, sections, start_va, end_va, label=""):
    ...
    out = [f"\n{'='*60}", f"Function: {label} ({start_va:08X} - {end_va:08X})", f"{'='*60}"]
...
def main():
    OUT_FILE = r"e:\WorkSpace\cncnet\Ra2.Vampire.Extend\disasm_funcs.txt"
    out = []
    ...
    # 0x50b6f0 - the "is engineer?" or "is player?" check
    out.extend(disasm_function(data, sections, 0x50b6f0, 0x50b780, "0x50b6f0 (called before spy check)"))
    
    # 0x4dded0 - the base class MouseOverObject call
    out.extend(disasm_function(data, sections, 0x4dded0, 0x4ddf60, "0x4dded0 (base MouseOverObject)"))
    
    # 0x4f9a90 - HouseClass::IsAlliedWith(ObjectClass*)
    out.extend(disasm_function(data, sections, 0x4f9a90, 0x4f9af0, "0x4f9a90 (IsAlliedWith?)"))
    
    # 0x6f3970 - called at 0x51E6DC
    out.extend(disasm_function(data, sections, 0x6f3970, 0x6f39e0, "0x6f3970"))
